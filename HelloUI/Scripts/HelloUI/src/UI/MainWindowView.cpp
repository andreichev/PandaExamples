...
MainWindowView::MainWindowView(Action onClick, Action onOpenWindow) {
    ...
    m_scrollView->layout().setFlexGrow(1.f);
    m_scrollView->layout().setWidth(PandaUI::Length::percent(100.f));
    m_scrollView->getContentView()->layout().setFlexDirection(PandaUI::FlexDirection::Column);
    m_scrollView->getContentView()->layout().setPadding(24.f);
    m_scrollView->getContentView()->layout().setGap(16.f);

    m_scrollView->addContentSubview(std::make_shared<MainHeaderView>());
    m_scrollView->addContentSubview(
        std::make_shared<MainToolbarView>(std::move(onClick), std::move(onOpenWindow))
    );
    ...
    m_scrollView->addContentSubview(std::make_shared<DemoCardView>(
        "Layout", "Flex direction, padding, gap, intrinsic height and grow are active here."
    ));
    m_scrollView->addContentSubview(std::make_shared<DemoCardView>(
        "Input", "Buttons use retained callbacks and keep state between frames."
    ));
    m_scrollView->addContentSubview(std::make_shared<DemoCardView>(
        "ScrollView", "The whole screen is inside one global PandaUI ScrollView."
    ));
    m_scrollView->addContentSubview(std::make_shared<DemoCardView>(
        "Runtime windows", "The Open window button creates a separate native PandaUI window."
    ));
    m_scrollView->addContentSubview(std::make_shared<DemoCardView>(
        "SDK path",
...
void MainWindowView::setClickCount(int clickCount) {
    if (m_statusLabel) {
        m_statusLabel->setText("Button clicks: " + std::to_string(clickCount));
    }
}
