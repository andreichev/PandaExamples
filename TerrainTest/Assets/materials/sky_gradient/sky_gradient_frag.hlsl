...
// Отличие от Neverland-оригинала: здесь нет скрипта SunCycle, который писал бы
// sunDirectionDay, поэтому направление солнца берётся из света СЦЕНЫ (префикс блоба
// SCENE_LIGHTS; submitSky биндит его шейдерам, объявившим блок — рефлексия). Диск на
// небе всегда совпадает с DirectionalLight: тени, просвет листвы и солнце — один источник.
cbuffer SCENE_LIGHTS {
    float4 ambientColorIntensity;
    float4 directionalDirectionIntensity; // xyz — К солнцу, w — интенсивность (0 = нет)
...
PSOutput main(PSInput input) {
    ...
    [branch]
    if (sunIntensity > 0.0 && dayAmount > 0.0 && directionalDirectionIntensity.w > 0.0) {
        float3 sunDirection = normalize(directionalDirectionIntensity.xyz);
